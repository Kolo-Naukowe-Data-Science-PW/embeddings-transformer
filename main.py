...
from midi_embeddings.transformer import generate_causal_mask, MIDITransformerEncoder
...
def train_model(model, train_dataset, val_dataset, config, device, model_name="model"):
    # Create dataloaders
    ...
    for epoch in range(config['epochs']):
        ...
        print(f"Epoch {epoch+1}/{config['epochs']} - Train Loss: {train_loss:.4f}, Val Loss: {val_loss:.4f}, Perplexity: {perplexity:.4f}")
...
def main():
    ...
    # Train model
    model = train_model(model, train_dataset, val_dataset, config, device, model_name="model_2048")
